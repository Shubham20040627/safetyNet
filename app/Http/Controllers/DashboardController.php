...
class DashboardController extends Controller
{
    ...
    /**
     * Live stats polling endpoint — returns fresh DB counts bypassing cache.
    ...
    public function liveStats()
    {
        ...
        $total    = (clone $neighborhoodReports)->count();
        $pending  = (clone $neighborhoodReports)->where('status', 'pending')->count();
        $resolved = (clone $neighborhoodReports)->where('status', 'resolved')->count();
        $active   = (clone $neighborhoodReports)->whereIn('status', ['pending', 'investigating'])->count();

        // Latest 5 incidents (fresh, no cache)
        $latest = (clone $neighborhoodReports)
            ->with('user')
            ->latest()
            ->take(5)
            ->get()
        ...
        // Weekly trend (last 7 days — fresh)
        $weeklyTrend = (clone $neighborhoodReports)
        ...
        // Smart Insights Calculation (fresh)
        if ($total === 0) {
            $peakTime = 'N/A';
            $trendDirection = 'stable';
            $trendPercent = 0;
        ...
        // Bust cache so next full page load is also fresh
